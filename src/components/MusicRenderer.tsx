...
export const MusicRenderer: React.FC<Props> = ({ score }) => {
  ...
  const totalScrollDistance = svgWidth + viewportWidth; // Total distance to scroll (from start to completely off-screen)
  const scrollSpeed = totalScrollDistance / duration; // pixels per second
  ...
  useEffect(() => {
    const animate = (currentTime: number) => {
      ...
      // Calculate position based on elapsed time and constant speed
      const newOffset = elapsedTime * scrollSpeed;

      // Reset when we've completed the full duration
    ...
    if (isPlaying) {
      ...
      // Reset timing references when stopping
  ...
  }, [isPlaying, duration, scrollSpeed]);
      ...
      >
        ...
        >
          {score.parts.map((part, partIndex) => {
            ...
            let beats =
              firstMeasureAttrs?.time?.find((t) => t.beats)?.beats ?? 4;
            let beatType =
              firstMeasureAttrs?.time?.find((t) => t.beatType)?.beatType ?? 4;
            const divisions = firstMeasureAttrs?.divisions ?? 1;
                    ...
                    notation.slur.forEach((slur) => {
                      ...
                      if (slur.type === "start") {
                        ...
                        activeSlurs.set(slurKey, {
                          startX: currentX + 6,
                          startY:
                            slurY + yOffset * (note.stem === "up" ? 1 : -1),
              ...
              <g key={`part-${partIndex}`}>
                ...
                {part.measures.map((measure, measureIndex) => {
                  ...
                  measure.elements.forEach((element, elementIndex) => {
                    ...
                    if (element.backup) {
                      currentX -=
                        element.backup.duration * DURATION_SPACING_UNIT;
                    }
                  });
